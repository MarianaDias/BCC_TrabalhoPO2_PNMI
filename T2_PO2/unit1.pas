...
begin
  ...
  if (i <> j) then
    begin
      ...
      FxRn(func, x, false, y2);
      x[i] := xi-h;
      FxRn(func, x, false, y4);
      x[j] := xj+h;
      FxRn(func, x, false, y3);
...
procedure Gauss(A: Matriz; b: Vetor; var x: Vetor);
var
  i,j,k : Integer;
  m, soma :Extended;
begin
 // Erro := True;
  ...
  for j:= 1 to naux-1 do
    begin
      for i:= j+1 to naux do
        begin
          if(A[j,j] = 0) then
            begin
              showMessage('Divisao por Zero - Escolha outro metodo');
              Exit;
            end
          else
             m := A[i,j]/A[j,j];
  ...
  //Solução por Retosubstituição
  x[naux] := b[naux]/A[naux,naux];
  for i := naux-1 downto 1 do
   begin
     soma := 0;
     for j:= i+1 to naux do
      begin
      soma := soma+ (A[i,j]*x[j]);
      end;
     x[i] := (b[i] -soma)/A[i,i];
